...
void loop() {
    ...
    if (bohleBNO.getImpact()) { // Check if a high_g event occurred (impact)
        Serial.println("Impact Detected!");
        bohleBNO.loadOffsets(100); // If yes, load the calibration values saved in EEPROM onto the BNO
        while (bohleBNO.getHeading() == 1); // Discard the first output from the BNO because it's most likely junk
    }
